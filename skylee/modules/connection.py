    ...
    if update.effective_chat.type == 'private':
        if args and len(args) >= 1:
            ...
            if conn:
                ...
                text = "You are currently connected to *{}* (`{}`)".format(connectedchat.title, conn)
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
            ...
            if connection_status:
                ...
                try:
                    ...
                    context.bot.send_message(update.effective_message.from_user.id, "You are connected to *{}*. \nUse `/helpconnect` to check available commands.".format(chat_name), parse_mode="markdown")
                except BadRequest:
                    pass
                except Unauthorized:
                    pass
