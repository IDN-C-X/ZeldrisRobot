...
def blacklist(update, context):
	...
	for text in split_text:
		if filter_list == "Current blacklisted words in <b>{}</b>:\n".format(chat_name):
			send_message(update.effective_message, "No blacklisted words in <b>{}</b>!".format(chat_name), parse_mode=ParseMode.HTML)
			return
...
def add_blacklist(update, context):
	...
	if len(words) > 1:
		...
		if len(to_blacklist) == 1:
			send_message(update.effective_message, "Added blacklist <code>{}</code> in chat: <b>{}</b>!".format(html.escape(to_blacklist[0]), chat_name),
...
def unblacklist(update, context):
	...
	if len(words) > 1:
		...
		if len(to_unblacklist) == 1:
			if successful:
				send_message(update.effective_message, "Removed <code>{}</code> from blacklist in <b>{}</b>!".format(html.escape(to_unblacklist[0]), chat_name),
							   parse_mode=ParseMode.HTML)
			else:
				send_message(update.effective_message, "This is not a blacklist trigger!")

		elif successful == len(to_unblacklist):
			send_message(update.effective_message, "Removed <code>{}</code> from blacklist in <b>{}</b>!".format(
			...
			send_message(update.effective_message, "Removed <code>{}</code> from blacklist. {} did not exist, "
...
Examples of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks."""
			...
			return ""
		if conn:
			text = "Changed blacklist mode: `{}` in *{}*!".format(settypeblacklist, chat_name)
		else:
		...
		if conn:
			text = ("Current blacklistmode: *{}* in *{}*.".format(settypeblacklist, chat_name))
		else:
